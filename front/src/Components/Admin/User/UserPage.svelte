<script lang="ts">
    ...
    async function load(_page: number) {
        loading = true;
        const data = await userApi.paging(_page, pageSize, text, sorting);
    ...
    function onCellClick(row: UserItem) {
    ...
    function onEditRow(id: number) {
        ...
        navigate("/admin/user/" + record?.username);
    ...
    function isRowSelected(row: any, selectedRows: UserItem[]) {
        let result = selectedRows.find((item) => item.id == row.id);
        return !!result;
    }
    function onSelectRow(event: any, row: UserItem) {
        ...
        <Table {loading} {rows} {pageIndex} {pageSize} let:rows={rows2}>
            ...
            <thead slot="head">
                <tr>
                    ...
                    <th>
                        username
                        <Sort key="username" on:sort={onSort} />
                    </th>
